...
/// Send and receive TAK messages (v0/CoT, v1_mesh, v1_stream) over TCP, TLS, or UDP
/// Usage: ./send_receive {address} {port} {tcp|tls|udp} {v0|v1_mesh|v1_stream}
/// E.g., ./send_receive 127.0.0.1 8087 tcp v0
///
...
/// 
/// A generic TAK event is sent every 5 seconds. The receiver prints any received
...
taktile::TakData gen_cot() {
  ...
  cot->set_staletime(taktile::TimeProvider::get_time(taktile::DEFAULT_COT_STALE));
  cot->set_lat(40.781789);  // set your lat (this loc points to Central Park NY)
  cot->set_lon(-73.968698);  // set your long (this loc points to Central Park NY)
  cot->set_hae(0);
  cot->set_ce(10);
  cot->set_le(10);
...
void receive_cb(taktile::TakData const& tak_data) {
  auto xml_msg = std::make_shared<sio::Message<XmlSerializer>>(std::move(tak_data.xml()));
...
int main(int argc, char** argv) {
  if (argc < 5) {
    std::cerr << "Usage: ./send_and_receive {address} {port} {tcp|tls|udp} {v0|v1_mesh|v1_stream}" << std::endl;
    return 1;
  }
  auto address = argv[1];
  ...
  if (protocol.compare("udp") == 0) {
    if (version.compare("v1_stream") == 0) {
      std::cerr << "V1 stream protocol is not supported for UDP transport." << std::endl;
      return 1;
    }
    auto serializer = std::make_shared<taktile::TakDataSerializerUdp>(use_v0_protocol);
    siotrnsip::Options options = siotrnsip::UdpOptions{.endpoint = siotrnsip::Endpoint{
  ...
  auto serializer = std::make_shared<taktile::TakDataSerializerTcp>(use_v0_protocol);
  std::shared_ptr<taktile::TakDataFramer> framer;
  
  if (version.compare("v0") == 0) {
    framer = std::make_shared<taktile::TakDataFramer>(taktile::ProtocolVersion::V0);
  } else if (version.compare("v1_mesh") == 0) {
    framer = std::make_shared<taktile::TakDataFramer>(taktile::ProtocolVersion::V1_MESH);
  } else {  // v1_stream
    framer = std::make_shared<taktile::TakDataFramer>(taktile::ProtocolVersion::V1_STREAM);
  }
