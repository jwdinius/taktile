...
/// values.
static inline auto contains_substring = [](std::string const& str,
                                           std::string_view const& sub_str) {
  return std::search(str.begin(), str.end(), sub_str.begin(), sub_str.end()) !=
         str.end();
};

static inline auto starts_with = [](std::string const& str,
                                    std::string_view const& prefix) {
  return str.size() >= prefix.size() &&
         str.compare(0, prefix.size(), prefix) == 0;
};

static inline auto ends_with = [](std::string const& str,
...
TEST(Types, TakData_XmlFromProto) {
  ...
  EXPECT_TRUE(std::regex_match(root->getAttribute("time"),
                               taktile::W3C_XML_DATETIME_REGEX));
  EXPECT_TRUE(std::regex_match(root->getAttribute("start"),
                               taktile::W3C_XML_DATETIME_REGEX));
  EXPECT_TRUE(std::regex_match(root->getAttribute("stale"),
                               taktile::W3C_XML_DATETIME_REGEX));

  auto point_element = root->getChildElement("point");
  EXPECT_NE(point_element, nullptr);
  EXPECT_EQ(point_element->getAttribute("lat"),
            fmt::format("{:.6f}", cot->lat()));
  EXPECT_EQ(point_element->getAttribute("lon"),
            fmt::format("{:.6f}", cot->lon()));
  EXPECT_EQ(point_element->getAttribute("le"),
            fmt::format("{:.6f}", cot->le()));
  EXPECT_EQ(point_element->getAttribute("hae"),
            fmt::format("{:.6f}", cot->hae()));
  EXPECT_EQ(point_element->getAttribute("ce"),
            fmt::format("{:.6f}", cot->ce()));
...
TEST(Types, TakData_ProtoFromHelloEvent) {
  ...
  auto tak_msg = std::make_shared<sio::Message<taktile::TakDataSerializerUdp>>(
      std::move(hello_event));
  {
  ...
  auto tak_msg_from_serialized =
      std::make_shared<sio::Message<taktile::TakDataSerializerUdp>>(
          std::move(serialized_tak_msg));
...
TEST(Types, TakMessage_V0_SerializeAndDeserialize) {
  ...
  auto tak_msg = std::make_shared<taktile::TakMessageUdp>(
      std::move(hello_event), serializer);
  {
    auto entity = tak_msg->entity();
    EXPECT_EQ(entity.proto().cotevent().uid(), "taco");
  }
  ...
  auto tak_msg_from_serialized = std::make_shared<taktile::TakMessageUdp>(
      std::move(serialized_tak_msg), serializer);
...
TEST(Types, TakMessage_V0_FrameAndUnframe) {
  ...
  EXPECT_TRUE(
      starts_with(framed_msg, std::string(taktile::V0_PROTOCOL_PREFIX)));
...
TEST(Types, TakMessage_V1_MESH_FrameAndUnframe) {
  ...
  // Frame the message
  auto framer = std::make_shared<taktile::TakDataFramer>(
  ...
  // suffix
  EXPECT_TRUE(
      starts_with(framed_msg, std::string(taktile::V1_MESH_PROTOCOL_PREFIX)));
...
TEST(Types, TakMessage_V1_STREAM_SerializeAndDeserialize) {
  ...
  auto tak_msg = std::make_shared<sio::Message<taktile::TakDataSerializerTcp>>(
      std::move(hello_event), serializer);
  {
    auto entity = tak_msg->entity();
    EXPECT_EQ(entity.proto().cotevent().uid(), "taco");
  ...
  auto tak_msg_from_serialized =
      std::make_shared<sio::Message<taktile::TakDataSerializerTcp>>(
          std::move(serialized_tak_msg), serializer);
